    ...
    if (command === '!theexpanse' && args.length) {
        try  {
            ...
            if (subtype === 'damage' && args.length === 5) {
                ...
                let d = [first_die, second_die, drama_die]
                response += expanseapi_card_hdr(name, 'Ability Test')
                response += expanseapi_card_test(attribute, total_modifiers, d,
                                         total + total_modifiers, tn, false)
            }
            else if (subtype === 'initiative' && args.length === 7) {
                let name = args.shift()
                let dice = args.shift()
                let attribute = args.shift()
                let attr_val = parseInt(eval(args.shift()))
...
function expanseapi_card_test( ability, modifier, dice, result, test_num, initiative ) {
    let out = ''
